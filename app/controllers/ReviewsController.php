...
class ReviewsController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				reviews.review_id LIKE ? OR 
				reviews.patient_id LIKE ? OR 
				reviews.tittle LIKE ? OR 
				reviews.message LIKE ? OR 
				reviews.doctor_id LIKE ? OR 
				reviews.date LIKE ?
			)";
			$search_params = array(
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("reviews.review_id", ORDER_TYPE);
		}
		if($fieldname){
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("reviews.review_id", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("patient_id","tittle","message","doctor_id","date");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("review_id","patient_id","tittle","message","doctor_id","date");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'patient_id' => 'required',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("reviews.review_id", $rec_id);;
	...
	function delete($rec_id = null){
		...
		$db->where("reviews.review_id", $arr_rec_id, "in");
